...
void ClientManager::privmsg(std::string &target_str, Client *client, std::string &message)
{
	std::ostringstream msg;

	std::vector<std::string> user = split(target_str, '@');
	if (user.size() > 1 && user[1] != Server::getHostname())
	{
		msg << ":" << Server::getHostname() << " 401 " << client->getNickname() << " " << target_str << " :No such nick/channel\r\n";
		client->sendMessage(msg);
		return ;
	}

	Client *target = find_nick(user[0]);
	if (target == NULL)
	{
		msg << ":" << Server::getHostname() << " 401 " << client->getNickname() << " " << target_str << " :No such nick/channel\r\n";
		client->sendMessage(msg);
	...
	msg << ":" << client->getNickname() << "!" << client->getUsername() << "@" 
		<< client->getHostname() << " PRIVMSG " << target->getNickname() << " :" << message << "\r\n";
