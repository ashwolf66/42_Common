...
void	Server::handle_nick(std::string &nickname, Client *client)
{
	...
	if (nickname.empty())
	{
		msg << ":" << Server::getHostname() << " 431 " << client->getNickname() << " :No nickname given\r\n";
		client->sendMessage(msg);
		return;
	}
...
void	Server::handle_privmsg(std::string &str, Client *client)
{
	...
	if (res.first[0] == '#')
	{
		ch_manager->privmsg(res.first, client, res.second);
		return;
	}
	Client *target = cm->find_nick(res.first);
	if (target == NULL)
	{
		msg << ":" << Server::getHostname() << " 401 " << client->getNickname() << " " << res.first << " :No such nick/channel\r\n";
		client->sendMessage(msg);
		return;
	}
	msg << ":" << target->getNickname() << " PRIVMSG " << res.first << " :" << res.second << "\r\n";
...
void Server::handle_whois(std::string &str, Client *client)
{
	if (client->getIsRegistered() == false)
		return send_not_registered(client);
	...
	Client *target = cm->find_nick(nick);
	if (target != NULL)
	{
		msg << ":" << Server::getHostname() << " 311 " << client->getNickname() << " " << target->getNickname()
			<< " ~" << target->getUsername() << " " << target->getHostname() << " * :" << target->getRealname() << "\r\n";
		client->sendMessage(msg);
	}
	else
	{
		msg << ":" << Server::getHostname() << " 401 " << client->getNickname() << " " << nick
			<< " :No such nick/channel\r\n";
		client->sendMessage(msg);
	}
}

void Server::handle_who(std::string &str, Client *client)
{
	if (client->getIsRegistered() == false)
		return send_not_registered(client);
	...
	ch_manager->who(client, channel, res.second);
}