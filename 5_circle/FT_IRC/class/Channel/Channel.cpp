...
void Channel::join(Client *client, std::string &password, std::ostringstream &msg)
{
	std::ostringstream broadcast_msg;
	...
	broadcast_msg << ":" << client->getNickname() << "!" << client->getUsername() << "@" << client->getHostname() << " JOIN :" << _Name << "\r\n";
	broadcast(client, broadcast_msg);
}
...
bool Channel::setFlag(Client *client, std::string &flag, char &toggle_sign, std::vector<std::string>::iterator &param_it, std::vector<std::string> &params, std::ostringstream &msg)
{
...
bool	Channel::selectMode(Client *client, std::string &flag, const std::string &param, std::ostringstream &msg)
{
	if (flag[1] == 'i')
		return I_Option(client, flag, msg);
	if (flag[1] == 't')
		return T_Option(client, flag, msg);
	if (flag[1] == 'k')
		return K_Option(client, flag, param, msg);
	if (flag[1] == 'o')
		return O_Option(client, flag, param, msg);
	return L_Option(client, flag, param, msg);
}
...
void Channel::mode(Client *client, std::vector<std::string> flags, std::vector<std::string> params)
{
	...
	for (std::size_t i = 0; i < flags.size(); ++i)
	{
		...
		if (setFlag(client, flag, toggle_sign, param_it, params, msg) == false || \
			selectMode(client, flag, param, msg) == false)
			continue;
	...
	if (apply_mode.empty() == false)
	{
		broadcast_msg << ":" << client->getNickname() << "!" << client->getUsername() << "@" << client->getHostname() << " MODE " << _Name;
		if (param_pair.second.empty())
			broadcast_msg << " :" << apply_mode << "\r\n";
		else
			broadcast_msg << " " << apply_mode << param_pair.first << " :" << param_pair.second << "\r\n";
		msg << broadcast_msg.str();
		broadcast(client, broadcast_msg);
	}
