...
void calculate_wall(t_data *data, t_ray *ray)
{
	...
	if (ray->side == 0)
	{
		ray->wall_dist = (ray->map_x - px + (1 - ray->step_x) / 2.0) / ray->ray_dir_x;
	}
	else
	{
		ray->wall_dist = (ray->map_y - py + (1 - ray->step_y) / 2.0) / ray->ray_dir_y;
	}
}
...
void draw_wall(t_data *data, t_ray *ray, int x)
{
	...
	while (y < ray->draw_end)
	{
		texture = get_texture(&data->map->texture[t_num].img,
							  t_x, ((y - WIN_HEIGHT / 2 + ray->height / 2) * data->map->texture[t_num].img.height) / ray->height);
